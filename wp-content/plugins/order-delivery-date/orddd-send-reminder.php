	...
	class orddd_send_reminder {
		...
		public static function orddd_reminder_email_before_days_callback( $args ) {
			...
			if ( $reminder_email_before_days > 0 ) {
				if ( ! wp_next_scheduled( 'orddd_auto_reminder_emails' ) ) {
					wp_schedule_event( $reminder_time, 'daily', 'orddd_auto_reminder_emails' );
				}
			} else {
		...
		public static function orddd_reminder_email_admin_callback( $args ) {
			...
			if ( get_option( 'orddd_reminder_for_admin' ) == 'on' ) {
				$checked = 'checked';
				if ( ! wp_next_scheduled( 'orddd_auto_reminder_emails_admin' ) ) {
					wp_schedule_event( $reminder_time, 'daily', 'orddd_auto_reminder_emails_admin' );
				}
			} else {
		...
		public static function orddd_modify_admin_reminder_email() {
			...
			if ( get_option( 'orddd_reminder_for_admin' ) == 'on'
					&& wp_next_scheduled( 'orddd_auto_reminder_emails_admin' ) != strtotime( '19:00:00 ' . $timezone )
					&& 'yes' != $time_changed ) {

				wp_clear_scheduled_hook( 'orddd_auto_reminder_emails_admin' );
				wp_schedule_event( strtotime( '19:00:00 ' . $timezone ), 'daily', 'orddd_auto_reminder_emails_admin' );
