			    ...
			    if ( get_option( 'orddd_enable_time_slot' ) == 'on' ) {
		                            ...
		                            if( $v->lockout != "" && $v->lockout != "0" ) {
		                                if( $v->tv == 'weekdays' ) {
		                                    if( 'all' == $dval ) {
		                                        ...
		                                        if ( array_key_exists( $dval, $delivery_days ) ) {
		                                            $previous_lockout =  $delivery_days[ $dval ] + $v->lockout;
		                                            $delivery_days[ $dval ] = $previous_lockout;
		                                        } else {
		                                            $delivery_days[ $dval ] = $v->lockout;
		                                        }
		                                    }
		                                } else {
		                                    if ( array_key_exists( $dval, $specific_dates ) ) {
		                                        $previous_lockout = $specific_dates[ $dval ] + $v->lockout;
		                                        $specific_dates[ $dval ] = $previous_lockout;
		                                    } else {
		                                        $specific_dates[ $dval ] = $v->lockout;
		                                    }
		                                }
		                            } else {
		                    ...
		                    if( $v->lockout != "" && $v->lockout != "0" ) {
		                        if( $v->tv == 'weekdays' ) {
		                        	if( 'all' == $v->dd ) {
			                            ...
			                            if ( array_key_exists( $v->dd, $delivery_days ) ) {
			                                $previous_lockout =  $delivery_days[ $v->dd ] + $v->lockout;
			                                $delivery_days[ $dval ] = $previous_lockout;
			                            } else {
			                                $delivery_days[ $dval ] = $v->lockout;
			                            }
		                            ...
		                            if ( array_key_exists( $v->dd, $specific_dates ) ) {
		                                $previous_lockout = $specific_dates[ $v->dd ] + $v->lockout;
		                                $specific_dates[ $dval ] = $previous_lockout;
		                            } else {
		                                $specific_dates[ $dval ] = $v->lockout;
		                            }
		            ...
		            if ( is_array( $delivery_days ) &&  count( $delivery_days ) > 0 ) {;
		                foreach( $delivery_days as $del_k => $del_v ) {
		                    $delivery_days[ $del_k ] = $del_v + $all_lockout;
		                }
		            ...
		            foreach ( $lockout_timeslots_arr as $k => $v ) {
		                ...
		                if ( array_key_exists( $date_str, $date ) ) {
		                    $previous_orders =  $date[ $date_str ] + $v->o;
		            ...
		            foreach ( $date as $k => $v ) {
		                if ( is_array( $specific_dates ) && count( $specific_dates ) > 0 && get_option( 'orddd_enable_specific_delivery_dates' ) == 'on' ) {
		                    ...
		                    if ( isset( $specific_dates[ $specific_date ] ) && $v >= $specific_dates[ $specific_date ] && $specific_dates[ $specific_date ] != '' && $blank_lockout_time_slot != "yes" ) {
		                        $lockout_arr[] = $k;
		                    } else if ( $v >= get_option( 'orddd_lockout_date_after_orders' ) && get_option( 'orddd_lockout_date_after_orders' ) != '' && get_option( 'orddd_lockout_date_after_orders' ) != '0' ) {
		                        $lockout_arr[] = $k;
		                    } else if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 ) {
		                        $weekday = date( 'w', strtotime( $k ) );
		                        if ( ( isset( $delivery_days[ 'orddd_weekday_' . $weekday ] ) && $v >= $delivery_days[ 'orddd_weekday_' . $weekday ] && $delivery_days[ 'orddd_weekday_' . $weekday ] != '' && $delivery_days[ 'orddd_weekday_' . $weekday ] != '0' ) ||
		                        ( isset( $delivery_days[ 'all' ] ) && $v >= $delivery_days[ 'all' ] && $delivery_days[ 'all' ] != '' && $delivery_days[ 'all' ] != '0' ) &&  $blank_lockout_time_slot != 'yes' ) {
		                             $lockout_arr[] = $k;
		                        } else if ( $v >= get_option( 'orddd_lockout_date_after_orders' ) && get_option( 'orddd_lockout_date_after_orders' ) != '' && get_option( 'orddd_lockout_date_after_orders' ) != '0' ) {
		                            $lockout_arr[] = $k;
		                        }
		                ...
		                if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 ) {
		                    ...
		                    if ( isset( $delivery_days[ 'orddd_weekday_' . $weekday ] ) ) {
		                        if( $v >= $delivery_days[ 'orddd_weekday_' . $weekday ] && '' != $delivery_days[ 'orddd_weekday_' . $weekday ]  && '0' != $delivery_days[ 'orddd_weekday_' . $weekday ] && 'yes' != $blank_lockout_time_slot) {
		                            if( !in_array( $k, $lockout_arr ) ) {
		                                $lockout_arr[] = $k;
		                            }
		                        }
						...
						if( is_array( $lockout_days_arr ) && count( $lockout_days_arr ) > 0 ) {
							foreach ( $lockout_days_arr as $k => $v ) {
								if ( $v->d == $date && $v->o >= $max_orders && $max_orders != '') {
									...
									if( $lockout_date_timestamp > $current_time ) {
            ...
            foreach( $time_slots as $tk => $tv ) {
                if( $tv != '' ) {
					...
					if( isset( $timeslot_values[ 'time_slot' ] ) ) {
						...
						$fm = date( "i", strtotime( $current_date . "00:" . $to_time_explode[ 1 ] ) );
					}
            		...
            		if( is_array( $timeslot_values[ 'selected_days' ] ) ) {
            			if ( $timeslot_values[ 'delivery_days_selected' ] == 'weekdays' ) {
            				foreach( $timeslot_values[ 'selected_days' ] as $dkey => $dval ) {
            					if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
            						if( 'all' == $dval ) {
            							...
            							if ( array_key_exists( $dval, $delivery_days ) ) {
            								$previous_lockout =  $delivery_days[ $dval ] + $timeslot_values[ 'lockout' ];
            								$delivery_days[ $dval ] = $previous_lockout;
            							} else {
            								$delivery_days[ $dval ] = $timeslot_values[ 'lockout' ];
            							}
            						}
            						...
            						if( 'all' == $dval ) {
            							...
            							if ( array_key_exists( $dval, $delivery_days ) ) {
            								$previous_lockout =  $delivery_days[ $dval ] + get_option( 'orddd_global_lockout_time_slots' );
            								$delivery_days[ $dval ] = $previous_lockout;
            							} else {
            								$delivery_days[ $dval ] = get_option( 'orddd_global_lockout_time_slots' );
            							}
            						}
            					} else {
            						$blank_lockout_time_slot = "yes";
            				...
            				foreach( $timeslot_values[ 'selected_days' ] as $dkey => $dval ){
            					if ( array_key_exists( $dval, $specific_dates ) ) {
            						if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
            							$previous_lockout =  $specific_dates[ $dval ] + $timeslot_values[ 'lockout' ];
            						} else if ( get_option( 'orddd_global_lockout_time_slots' ) != '0' && get_option( 'orddd_global_lockout_time_slots' ) != '' ) {
            							$previous_lockout =  $specific_dates[ $dval ] + get_option( 'orddd_global_lockout_time_slots' );
            						} else {
            							$blank_lockout_time_slot = "yes";
            						}
            						$specific_dates[ $dval ] = $previous_lockout;
            						...
            						if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
            							$specific_dates[ $dval ] = $timeslot_values[ 'lockout' ];
            						} else if ( get_option( 'orddd_global_lockout_time_slots' ) != '0' && get_option( 'orddd_global_lockout_time_slots' ) != '' ) {
            							$specific_dates[ $dval ] = get_option( 'orddd_global_lockout_time_slots' );
            						} else {
            							$blank_lockout_time_slot = "yes";
            						}
            					}
            ...
            foreach ( $date as $k => $v ) {
            	...
            	if ( is_array( $specific_dates ) && count( $specific_dates ) > 0 && isset( $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] ) &&  $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] == 'on' ) {
            		if ( isset( $specific_dates[ $specific_date ] ) && $v >= $specific_dates[ $specific_date ] && $specific_dates[ $specific_date ] != '' && $blank_lockout_time_slot != "yes" ) {
            			...
            			$is_specific = 'yes';
            		} else if ( $v >= $shipping_settings[ 'date_lockout' ] && $shipping_settings[ 'date_lockout' ] != '' && $shipping_settings[ 'date_lockout' ] != '0' ) {
            			if( !in_array( $k, $lockout_arr ) ) {
            				$lockout_arr[] = $k;
            			}
            		} else if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 && !isset( $specific_dates[ $specific_date ] ) ) {
            			$weekday = date( 'w', strtotime( $k ) );
            			if ( ( isset( $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] ) && $v >= $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] && $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] != '' && $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] != '0' ) ||
            				( isset( $delivery_days[ 'all' ] ) && $v >= $delivery_days[ 'all' ] && $delivery_days[ 'all' ] != '' && $delivery_days[ 'all' ] != '0' ) &&  $blank_lockout_time_slot != 'yes' ) {
            					if( !in_array( $k, $lockout_arr ) ) {
            						$lockout_arr[] = $k;
            					}
            	...
            	if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 && !isset( $specific_dates[ $specific_date ] ) ) {
            		...
            		if ( isset( $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] ) ) {
            		    if( $v >= $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] && '' != $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ]  && '0' != $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] && 'yes' != $blank_lockout_time_slot) {
                		    if( !in_array( $k, $lockout_arr ) ) {
                		        $lockout_arr[] = $k;
                		    }
            		    }
                ...
                if( $i >= 0 ) {
                	...
                	if( is_array( $shipping_settings ) && count( $shipping_settings ) > 0 ) {
                		if( 'on' == get_option( 'orddd_enable_shipping_days' ) ) {
							...
							//if the shipping weekdays are disabled then check for global weekdays.
							
	                		if ( !isset( $weekdays_to_check[ "orddd_weekday_" . $current_weekday ] ) ) {
	                			$weekday_disabled = "yes";
	                		}
	                	}
                	} else {
