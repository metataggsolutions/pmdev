...
add_filter( 'cron_schedules', 'orddd_add_import_events_cron_schedule' );

/**
 * Adds the Order Delivery Date Import Cron Job
 *
 * @param array $schedules - List of cron jobs to be run with their intervals
 * @since 9.1
 */

function orddd_add_import_events_cron_schedule( $schedules ) {

    // Fetch the import frequency in minutes if the checkbox is enabled else the time in 24 hours
    ...
    $schedules[ 'orddd_gcal_import' ] = array(
        'interval' => $orddd_wp_cron_seconds, // import duration in seconds
        'display'  => __( 'Order Delivery Date - GCal Import Events' ),
    );
    return $schedules;
}
...
if ( ! wp_next_scheduled( 'orddd_import_events' ) ) {
    wp_schedule_event( time(), 'orddd_gcal_import', 'orddd_import_events' );
}
...
/**
 * Schedule an action to clean the database options for gcal events. This action will be scheduled * at 1 AM daily.
 * 
 * @since 9.7
 */
if ( ! wp_next_scheduled( 'orddd_clean_events_db' ) ) {
    wp_schedule_event( strtotime('01:00:00'), 'daily', 'orddd_clean_events_db' );
}
    ...
    */
    ...
    function orddd_add_to_calendar_customer( $order ) {
        ...
        if ( isset( $data[ '_orddd_timestamp' ][ 0 ] ) && $data[ '_orddd_timestamp' ][ 0 ] != '' ) {
            ...
            $orddd->order_total  = $order->get_formatted_order_total();
            ...
            if( get_option( 'orddd_calendar_in_same_window' ) == 'on' ) {
                ...
                <div class="add_to_calendar">
                    ...
                    <div id="add_to_calendar_menu" class="add_to_calendar-content">
                        <a href="<?php echo $href; ?>" target= "_self" id="add_to_google_calendar" ><img class="icon" src="<?php echo plugins_url(); ?>/order-delivery-date/images/google-icon.ico"><?php _e( 'Add to Google Calendar', 'order-delivery-date' ); ?></a>
                        <a href="<?php echo $other_calendar_href; ?>" target="_self" id="add_to_other_calendar" ><img class="icon" src="<?php echo plugins_url(); ?>/order-delivery-date/images/calendar-icon.ico"><?php _e( 'Add to other Calendar', 'order-delivery-date' ); ?></a>
                    </div>
                </div>
                <?php 
            } else {
                ...
                <div class="add_to_calendar">
                    <button onclick="myFunction()" class="dropbtn"><?php _e( 'Add To Calendar', 'order-delivery-date' ); ?><i class="claret"></i></button>
                    <div id="add_to_calendar_menu" class="add_to_calendar-content">
                        <a href="<?php echo $href; ?>" target= "_blank" id="add_to_google_calendar" ><img class="icon" src="<?php echo plugins_url(); ?>/order-delivery-date/images/google-icon.ico"><?php _e( 'Add to Google Calendar', 'order-delivery-date' ); ?></a>
                        <a href="<?php echo $other_calendar_href; ?>" target="_blank" id="add_to_other_calendar" ><img class="icon" src="<?php echo plugins_url(); ?>/order-delivery-date/images/calendar-icon.ico"><?php _e( 'Add to other Calendar', 'order-delivery-date' ); ?></a>
                    </div>
                </div>
                <?php 
            }
