    ...
    */ 
    ...
    public function order_delivery_date_admin_settings() {
        ...
        add_settings_field(
            ...
            'orddd_date_settings_page',
            'orddd_date_settings_section',
            array ( __( 'Select weekdays for deliveries.', 'order-delivery-date' ) )
        );
        ...
        add_settings_field(
            ...
            'orddd_date_settings_page',
            'orddd_shipping_days_settings_section',
            array ( __( 'Calculate Minimum Delivery Time (in hours), Same Day cut-off and Next Day cut-off based on the shipping days selected.', 'order-delivery-date' ) )
        );
        ...
        add_settings_field(
            ...
            'orddd_date_settings_page',
            'orddd_shipping_days_settings_section',
            array ( __( 'Select weekdays for shipping.', 'order-delivery-date' ) )
        );
	    ...
	    if ( ( isset( $_GET[ 'page' ] ) && sanitize_text_field( $_GET[ 'page' ] ) == 'order_delivery_date' ) && 
	        ...
	        if( ( isset( $_GET[ 'action' ] ) && sanitize_text_field( $_GET[ 'action' ] ) == 'orddd_delete' ) || 
    	        ...
    	        foreach( $delivery_date as $d_key => $d_value ) {
    	            ...
    	            // Lockout Days
    	            $lockout_days = json_decode( get_option( 'orddd_lockout_days' ) );
    	            if ( $lockout_days == '' || $lockout_days == null ) {
    	                $lockout_days = array();
    	            }
    	            $i = 0;
    	            $new_arr_order = array();
    	            foreach ( $lockout_days as $k => $v ) {
    	                if ( $v->d == $d_value ) {
    	                    $delete_status = "Y";
    	                    unset( $lockout_days[ $k ] );
    	                }
    	                if ( $delete_status == "N" ) {
    	                    $new_arr_order[ $i ] = $v;
    	                    $i++;
    	            ...
    	            $lockout_days = $new_arr_order;
    	            update_option( 'orddd_lockout_days',json_encode( $lockout_days ) );
